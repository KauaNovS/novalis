...
export default function EditDeveloperPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Atualizar dados da incorporadora
      ...
      if (!res.ok) throw new Error(data.error || "Erro ao atualizar incorporadora");

      // 2. Se houver nova logo, fazer upload
      if (logoFile) {
        const formData = new FormData();
        formData.append("file", logoFile);

        const uploadRes = await fetch(`/api/developers/${id}/logo`, {
          method: "POST",
          body: formData,
        });
    ...
    <main className="min-h-screen bg-[#FAFAFA] p-8">
      <div className="max-w-2xl mx-auto">
        <div className="mb-6">
          ...
          >
            ← Voltar para incorporadoras
          </Link>
        </div>

        <h1 className="text-2xl font-bold text-gray-900 mb-6">
          Editar Incorporadora
        </h1>
